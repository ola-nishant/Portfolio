...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        notify("An error occurred. Please try again later.", true);
      }
    }
  };
  ...
  return (
    <div
      className="flex flex-col gap-8 mx-auto w-full bg-black xl:rounded-large md:rounded-medium rounded-small items-center md:my-[90px] my-[70px] py-[50px] px-4"
      id="Hero"
    >
      <h1 className="2xl:text-[56px] md:text-[41px] text-[28px] leading-hero md:text-nowrap tracking-tighter font-medium text-white">
        Let&apos;s get in touch!
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:w-[60%] gap-2">
        ...
        <textarea
          ...
          className={`block p-4 w-full text-sm text-gray-900 bg-gray-50 rounded-[16px] ${
            errors.message ? "border-3 border-red-500" : ""
          }`}
          placeholder="Message"
          value={form.message}
          onChange={handleChange}
        />
        {errors.message && (
          <span className="text-red-500 text-sm relative bottom-2">
            {errors.message}
          </span>
        )}
